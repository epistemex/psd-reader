...
PsdReader.prototype._parser = function(buffer) {
	...
	// check reserved space (6 next bytes should be 0)
	...
	width = getUint32();
	if (width < 1 || width > 3e4 || height < 1 || height > 3e4) {
		_err("Invalid size");
		return
	}
...
/*	if (mode > 15) {
	...
	if (mode > 15 || [5,6,11,12,13,14,15].indexOf(mode) > -1) {
	...
	modeDesc = [
		"Bitmap", "Greyscale", "Indexed", "RGB", "CMYK", "", "",
		"Multichannel", "Duotone", "Lab", "Greyscale16", "","",
		"","",""][mode];
	...
	// Image Data section
	addChunk("ImageData", view.byteLength - pos);
